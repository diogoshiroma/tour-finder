...
import {
  mapHostDataToHost,
  mapBedroomDataToBedroom,
  mapHostAndBedroomToResidence,
  parseResidenceToLodginEvent,
  parseBedroomToBedroomData,
  parseResidenceToBedroom,
  mapDataToTourData,
  mapTourDataToTour
} from "./mapper";
...
export const getTours = async (
  ...
  const axiosResponse: AxiosResponse<any> = await axios.put(url, {
    'city': city,
    'date': date,
    'available': quantity,
    'tour_type': type
  });
  console.log(city, date, quantity, type)
...
const parseBedroomsArrayData = (bedroomsData: any): Bedroom[] => {
  return bedroomsData.data.map((bedroomData: any) =>
    mapBedroomDataToBedroom(bedroomData)
  );
};
...
export const getResidences = async (): Promise<Residence[]> => {
  ...
  const bedroomPromises: Promise<Bedroom[]>[] = hosts.map(host =>
    getBedroomsOfHost(host.id)
  );
  return Promise.all(bedroomPromises)
    .then(function(bedroomLists) {
      const bedrooms: Bedroom[] = bedroomLists.flat();
      const residencesFromBedrooms = bedrooms.map(bedroom =>
        mapHostAndBedroomToResidence(
          bedroom,
          hosts.filter(host => host.id === bedroom.hostId)[0]
        )
      );
...
export const addLodgingEvent = async (
  residence: Residence,
  startDate: Date,
  endDate: Date
) => {
  const url = `${TRAVEL_TOUR_API_URL}/api/lodgingevents/`;
  const lodgingEvent: LodginEvent = parseResidenceToLodginEvent(
    residence,
    startDate,
    endDate
  );
